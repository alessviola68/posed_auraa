...
## 19. Clarifications GitHub, Vercel et workflow

Contexte confirme cote utilisateur apres le test Codex Web :

- La branche de production GitHub est `main`.
- Vercel est connecte au repository GitHub du projet.
- Vercel redeploie automatiquement apres un merge sur `main`.
- Les previews Vercel sont activees pour les Pull Requests.
- Le test avec `CODEX_PR_TEST.md` a bien cree une Pull Request GitHub et une preview Vercel automatiquement.
- Le fichier `CODEX_PR_TEST.md` etait uniquement temporaire pour verifier la creation de Pull Request et ne doit pas rester dans le projet.

Regles de workflow confirmees :

- Privilegier de petites Pull Requests ciblees.
- Idealement : 1 objectif clair = 1 Pull Request.
- Exemples d'objectifs separes : modifier les tarifs, ameliorer la galerie, ajouter une FAQ, optimiser les medias.
- Des lots plus larges sont acceptables uniquement si les sujets sont coherents entre eux, par exemple :
  - galerie + optimisation medias ;
  - tarifs + texte de reservation ;
  - responsive + corrections UI mineures.
- Ne pas melanger trop de sujets dans une meme Pull Request.
- Toujours eviter les refontes larges sans validation explicite.

Points de contenu confirmes :

- Les tarifs actuels restent provisoires tant que la cliente ne les a pas valides.
- Le lien Instagram a utiliser est `https://www.instagram.com/posed_auraa/`.
- Le lien Instagram devra quand meme etre reverifie une derniere fois cote cliente avant publication finale.
